...
router.post("/signup", (req, res) => {
	...
	connection.query(signUpQuery, [user.email], (err, results) => {
		if (!err) {
			//If user email does not exist in DB insert
			if (results.length <= 0) {
				signUpQuery =
...
router.post("/login", (req, res) => {
	...
	connection.query(loginQuery, [user.email], (err, results) => {
		if (!err) {
			if (results.length <= 0 || results[0].password != user.password) {
				...
				// If login its correct send token
			} else if (results[0].password == user.password) {
				const response = { email: results[0].email, role: results[0].role };
				//const accesToken = jwt.sign(response, process.env.ACCESS_TOKEN, {
				const accesToken = jwt.sign(response, process.env.ACCESS_TOKEN, {
					expiresIn: "8h",
				});
...
router.post("/forgotpassword", (req, res) => {
	const user = req.body;
	forgotPasswordQuery = "select email,password from user where email=?";
	connection.query(forgotPasswordQuery, [user.email], (err, results) => {
		if (!err) {
			// User not found
			if (results.length <= 0) {
				return res
				...
				var mailOptions = {
					...
					html:
						...
						"<br><b>Password: </b>" +
						results[0].password +
						'<br><a href="http://localhost:9999/">Click for login</a></p>',
				};
...
router.patch(
	...
	(req, res) => {
		let user = req.body;
		var updateStatusQuery = "update user set status=? where id=?";
		connection.query(
			updateStatusQuery,
			[user.status, user.id],
			(err, results) => {
...
router.post("/changePassword", auth.authenticateToken, (req, res) => {
	...
	var oldPasswordQuery = "select * from user where email=? and password=?";
	connection.query(
		oldPasswordQuery,
		[email, user.oldPassword],
		(err, results) => {
			if (!err) {
				if (results.length <= 0) {
